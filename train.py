...
def train():
    ...
    print_freq = 10  # print avg reward in the interval (in num episodes)
    ...
    print("max training episodes : ", max_episodes)
    ...
    for i_episode in range(1, max_episodes + 1):
        ...
        for t in range(1, max_ep_len + 1):
            ...
            if time_step % update_timestep == 0:
                print("begin to update the ppo agent")
                ppo_agent.update()
        ...
        # save the gif
        if i_episode % save_gif_freq == 0:
            print("--------------------------------------------------------------------------------------------")
            print("saving gif at : " + gif_dir)
            state = env.reset()
            ep_reward = 0
            for t in range(1, max_ep_len + 1):
                action = ppo_agent.select_action(state)
                ...
                img.save(gif_images_dir + '/' + 'episode' + str(i_episode) + str(t).zfill(6) + '.jpg')

                if done:
                    break
            # ppo_agent.buffer.clear()
            print('Episode: {} \t\t Reward: {}'.format(i_episode, round(ep_reward, 2)))
            # print("total number of frames / timesteps / images saved : ", t)

            gif_path = gif_dir + '/PPO_' + env_name + 'episode' + str(i_episode) + '.gif'

            img_paths = sorted(glob.glob(gif_images_dir + '*.jpg'))
